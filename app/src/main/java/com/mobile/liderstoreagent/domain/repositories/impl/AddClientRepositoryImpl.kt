    ...
    override suspend fun addClient(data: AddClientData): Flow<Result<Pair<Int, MarketCode?>>> =
        flow {
            ...
            try {
                ...
                val marketName = data.name.toRequestBody("text/plain".toMediaTypeOrNull())
                val address = data.address.toRequestBody("text/plain".toMediaTypeOrNull())
                val responsiblePerson =
                    data.responsible_agent.toRequestBody("text/plain".toMediaTypeOrNull())
                val reqPhone1 = data.director_phone_number.toRequestBody("text/plain".toMediaTypeOrNull())
                val reqPhone2 = data.work_phone_number.toRequestBody("text/plain".toMediaTypeOrNull())
                val INN = data.INN.toRequestBody("text/plain".toMediaTypeOrNull())
                val director = data.director.toRequestBody("text/plain".toMediaTypeOrNull())
